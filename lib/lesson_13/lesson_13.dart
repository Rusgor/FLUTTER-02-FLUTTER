...
// Відповідь: Зелений контейнер накладається на червоний,
// тому що OverflowBox дозволяє child виходити за межі свого SizedBox.
// Але зелений контейнер не накладається на синій,
// тому що у Column діти малюються послідовно зверху вниз.

// Синій контейнер малюється після зеленого,
// тому він перекриває нижню частину зеленого контейнера.
//
// Тобто:
// - зелений overflow може виходити за межі свого layout space
// - але порядок малювання (paint order) у Column залишається незмінним
// - останні віджети малюються поверх попередніх.
...
// Відповідь: LimitedBox застосовує обмеження лише тоді,
// коли отримує unbounded constraints від батьківського віджета.
//
// У першому випадку Column передає unbounded height,
// тому maxHeight: 50 спрацьовує.
//
// У другому випадку SizedBox(height: 100)
// передає bounded constraints,
// тому LimitedBox ігнорує свої обмеження
